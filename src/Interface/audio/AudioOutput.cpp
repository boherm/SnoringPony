...
void AudioOutput::refreshChannelOptions()
{
    ...
    audioInterface->am.getAudioDeviceSetup(setup);
...
void AudioOutput::onContainerParameterChangedInternal(Parameter* p)
{
    ...
    if (p == chan1 && chan1->intValue() > 0) { savedChan1 = chan1->intValue(); chan1->clearWarning(); }
    if (p == chan2 && chan2->intValue() > 0) { savedChan2 = chan2->intValue(); chan1->clearWarning(); }
}
